...
import DataType from "../../core/utils/type";
 ...
 */
const _configResponseInterceptor = (
  instance,
  ...
  instance.interceptors.response.use(
    (res) => {
      if (responseInterceptorSuccessCb) {
        ...
        console.log(
          `%c${"[HTTP]"} %c${dayjs().format(
            "HH:mm:ss"
          )} %c${res.config.method.toLocaleUpperCase()} %c${
            "/" +
      ...
      console.log(
        ...
        "font-weight:bold",
        "font-weight:bold",
        `color:${res.status === 200 ? "#67C23A" : "#F56C6C"};font-weight:bold`,
        "font-weight:bold"
      );
      return res;
    },
      ...
      if (config.__retryCount >= config.retry) {
        ...
        if (responseInterceptorErrorCb) {
          ...
          if (!DataType.isObject(err?.config)) {
            throw Error(
              "responseInterceptorErrorCb必须返回一个err包含{config,response}"
            );
          }
